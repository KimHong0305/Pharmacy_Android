...
const ChooseCouponScreen = () => {
  ...
  const handleSelectCoupon = (id: string, orderRequire: number) => {
    if (totalPrice >= orderRequire) {
    ...
    <View style={styles.container}>
          ...
          return (
            <View key={item.id} style={[styles.card, isDisabled && styles.disabledCard]}>
              ...
              <CheckBox
                ...
                onPress={() => handleSelectCoupon(item.id, item.orderRequire)}
                checkedColor="blue"
      ...
      <View style={styles.footer}>
        <Button
          title="Áp dụng"
          onPress={() => {
            navigation.navigate('OrderCartScreen', { selectedCoupon: selectedCoupon ?? null });
          }}                
